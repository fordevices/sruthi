...
| Orchestrator + run logging | ✅ Complete | Session 6 |
...
**What gets built:** ANSI colour output, short-file guard, `--stats` top-albums line, two hardening checks, final README sections filled in.

**Verification:** `--stats` output, colour rendering in terminal, short-file guard on a <8s clip, `--review --flagged` on current DB confirms clean.

---

## Confirmed DB state entering Session 7

| song_id | language | status | notes |
|---|---|---|---|
| max-000001 to max-000022 | Tamil/Hindi/English | `done` (20) or `no_match` (2) | Original batch |
| max-000023 to max-000025 | any | `no_match` | Noise files from Session 6 Pass 2 |

Total: 25 songs. 20 `done`, 5 `no_match`. No `error` rows.
The 2 real no_match files are max-000014 (`Sathileelavathy_MarugoMarugo.mp3`)
and max-000020 (`sattam_naanbaneyennathuuyir.mp3`).

---

```
Read README.md before writing any code — including the "Confirmed DB state entering
Session 7" section immediately above, and the "Known issues / limitations" and
"Tips for Tamil / Hindi music matching" sections which are now filled in.

Session 7 goal: polish CLI output, add two hardening guards, run final verification.

───────────────────────────────────────────
ITEM 1 — ANSI colour output
───────────────────────────────────────────
Add to the top of pipeline/runner.py (not imported — raw codes only):

  import sys
  _C = sys.stdout.isatty()   # disable colour when piped or redirected
...
Apply to per-file output lines across all four stage modules.
Import and use the constants from runner.py — do not duplicate them:
  from pipeline.runner import GREEN, YELLOW, RED, RESET

Colour mapping:
  ✓ identified / ✓ tagged / ✓ moved  → GREEN
  ✗ no match / ⚠ too short           → YELLOW
  ✗ error                             → RED
  === Run headers ===                 → BOLD

───────────────────────────────────────────
ITEM 2 — Short-file guard in identify.py
───────────────────────────────────────────
After inserting the song row and before calling shazam.recognize():
...
Print: [max-000025] ⚠ too short   (3.2s) Tamil | clip.mp3

───────────────────────────────────────────
ITEM 3 — --stats top-albums line
───────────────────────────────────────────
Add a third block to the --stats output:

  Top albums (by song count):
    Rettai Vaal Kuruvi...          3
    Abhimaan                        2
    Unknown Album                   5

  2 files still need review — run: python main.py --review

Query for top 5 albums:
  SELECT COALESCE(NULLIF(final_album,''), shazam_album, 'Unknown Album') as album,
         COUNT(*) as n
  FROM songs GROUP BY album ORDER BY n DESC LIMIT 5

Show the "still need review" line only if any songs have status='no_match'.

───────────────────────────────────────────
ITEM 4 — Hardening checks (fix if wrong)
───────────────────────────────────────────
Check 4a: run_pipeline passes run_id to run_identification correctly.
  In runner.py, the call must be: run_identification(source_path, run_id)
  If the current call omits run_id or passes it as a kwarg with wrong name, fix it.
  Verify by confirming the run_id column in the songs table matches the run folder name.

Check 4b: --stage N --dry-run composes correctly for all stages.
  Test: python main.py Input/ --stage 3 --dry-run
  Should print dry-run preview lines, not actually write any tags.
  Test: python main.py Input/ --stage 4 --dry-run
  Should print "would move" lines, not actually move any files.
  Fix any case where dry_run=True is not threading through to the stage function.

───────────────────────────────────────────
VERIFICATION
───────────────────────────────────────────
Run these five checks and paste the output of each:

V1 — Stats with real DB
  python main.py --stats
  Expected: 25 total, 20 done, 5 no_match, top albums list, review reminder line.

V2 — Colour rendering (must be run in a real terminal, not piped)
  python main.py Input/ --stage 1
  (all 25 files will be skipped as already in DB — but the skip lines should render
  in the correct colour. Paste a screenshot description or the raw terminal text.)

V3 — Short-file guard
  Create a short clip: ffmpeg -f lavfi -i "sine=frequency=440:duration=3" -q:a 9 /tmp/short.mp3
  cp /tmp/short.mp3 Input/English/
  python main.py Input/English/ --stage 1
  Paste the output line for the short file — should show ⚠ too short (3.0s).
  Confirm status=no_match in DB for that song_id.

V4 — Dry-run composition
  python main.py Input/ --stage 3 --dry-run
  python main.py Input/ --stage 4 --dry-run
  Both should print preview output only. Paste the first 3 lines of each.

V5 — Flagged review on current DB
  python main.py --review --flagged
  Expected: no songs flagged (max-000015 Maharajanodu already corrected to 1995).
  Should print: "No flagged songs found." or similar.
  Paste the output.

After all five pass, update README.md:
  - Session 7 → ✅ Complete in the status table
  - Add today's date as "Project completed" line under the status table
  - The Known issues and Tips sections are already filled in (done above)
```
...
Confirmed across Sessions 2–6 on a 25-file real-world Tamil / Hindi / English batch:

- **2 unresolved no_match files** — `Sathileelavathy_MarugoMarugo.mp3` and
  `sattam_naanbaneyennathuuyir.mp3`. Garbled romanised filenames with no metadata.
  Shazam cannot fingerprint them correctly. Resolve via `python main.py --review`:
  listen to the file, Google the lyrics, enter `Title | Artist | Album | Year`.
- **Shazam year errors exist in the wild** — `Maharajanodu` returned year `1905`
  (correct: 1995). Run `python main.py --review --flagged` after every large batch
  to surface any year before ~1940, then correct with the partial-override syntax
  `| | | 1995` (blank fields preserve existing values).
- **Album missing on ~10% of tracks** — Shazam's metadata for some recordings has
  no album entry in `sections.metadata`. Falls back correctly to `Unknown Album/`.
  No action needed unless you want to fill it in manually via `--review --all`.
- **Cover art absent on manual-override songs** — when `override_used=1`, there is
  no `shazam_cover_url` (the song was never matched). The APIC tag is simply not
  written. This is expected behaviour.
- **Non-music audio (noise, speech, short clips) returns no_match** — correct.
  The pipeline inserts the file, attempts fingerprinting, gets no_match, then skips
  Stages 3 and 4. No manual intervention needed unless you want to tag them.
- **Shazam rate limiting** — not observed in testing at `SHAZAM_SLEEP_SEC = 2.0`.
  If you see repeated errors on a large batch, increase this value in `config.py`.

---

## Tips for Tamil / Hindi music matching

Based on real results from Sessions 2–6:

- **86% match rate on 1970s–2000s Tamil and Hindi film music** — better than expected.
  Shazam's Indian music coverage is strong for anything that was commercially released.
- **Ilaiyaraaja tracks from the 1980s match reliably** — Raja Raja Chozhan (1987),
  Mounamaananeram, Maharajanodu, Saathu Nada, and others all identified correctly.
- **Classic Hindi film music matches well** — Lata Mangeshkar, Kishore Kumar,
  Mukesh, Asha Bhosle all identified from 1970s–1990s recordings.
- **Garbled romanised filenames are the main no_match cause** — files named like
  `sattam_naanbaneyennathuuyir.mp3` have no metadata to help Shazam and the audio
  fingerprint may not be in the database. Workflow: `--review`, play the file,
  Google a few lyrics, enter the metadata.
- **Run `--review --flagged` after every large batch** — takes 30 seconds and catches
  any Shazam DB year errors before they propagate into your folder structure.
- **The `Unknown Year/` folder is a useful work queue** — any song that lands there
  after a run is one where Shazam had no year data. Review those specifically with
  `--review --all` to fill in the missing fields.
